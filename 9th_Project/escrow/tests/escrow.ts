...
describe("escrow", () => {
  ...
  before('Creates Alice and Bob accounts, 2 token mints, and associated token accounts for both tokens for both users', async () => {
    ...
    const createMintToInstructions: Array<TransactionInstruction> = [tokenMintA, tokenMintB].map((mint) =>
    ...
    tx.instructions = [...sendSolInstructions, ...createMintToInstructions, ...mintTokensInstructions]
